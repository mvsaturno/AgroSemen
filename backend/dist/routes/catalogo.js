...
async function catalogoRoutes(app) {
    ...
    app.get('/api/:token', async (request, reply) => {
        ...
        if (!whatsappDestino) {
            const primeiroUsuario = await database_1.default.usuario.findFirst({
    ...
    app.post('/api/:token/reservas', async (request, reply) => {
        ...
        const schema = zod_1.z.object({
            ...
            telefoneComprador: zod_1.z.string().min(1),
            fazendaComprador: zod_1.z.string().optional(),
        ...
        for (const item of parsed.data.itens) {
            const r = await database_1.default.intencaoReserva.create({
                data: {
        ...
        const mensagemOriginal = `*Pedido de Reserva - AgroSêmen*\n` +
            `----------------------------------\n` +
