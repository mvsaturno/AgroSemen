...
export default async function authRoutes(app: FastifyInstance) {
  ...
  app.post('/send-code', async (request, reply) => {
    ...
    // Envia o SMS
    const success = await SmsService.sendVerificationCode(telefone, code)
    if (!success) {
  ...
  app.post('/register', async (request, reply) => {
    const schema = z.object({
      nomeConta: z.string().min(2, 'Nome da conta obrigatório'),
      nomeUsuario: z.string().min(2, 'Nome obrigatório'),
      ...
      perfil: z.enum(['USO_PROPRIO', 'PRESTADOR']).optional().default('USO_PROPRIO'),
    })
  ...
  app.post('/invite', {
    ...
    const schema = z.object({
      nomeConvidado: z.string().min(2),
      telefoneConvidado: z.string().min(8),
    })
    ...
    const jwtPayload = request.user as { contaId: string }
  ...
  app.post('/activate/:token', async (request, reply) => {
    ...
    const schema = z.object({
      ...
      nome: z.string().min(2).optional(), // pode atualizar o nome
    })
  ...
  app.post('/refresh', { preHandler: authenticate }, async (request, reply) => {
    const user = request.user as { sub: string; contaId: string; papel: string; nome: string }
    const token = app.jwt.sign({
      sub: user.sub,
